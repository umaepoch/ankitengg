...
def get_period_list_test(from_fiscal_year, to_fiscal_year, periodicity, accumulated_values=False,
	...
	print("year_start_date",year_start_date)
	print("year_end_date",year_end_date)
	print("months",months)
	for i in range(cint(math.ceil(months / months_to_add))):
		...
		start_date = to_date
		print("to_dateaddmonths",to_date)
		# Subtract one day from to_date, as it may be first day in next fiscal year or month
		to_date = add_days(to_date, -1)
		print("to_dateadddays",to_date)
...
def prepare_data(accounts, balance_must_be, period_list, company_currency):
	...
	for d in accounts:
		...
		for period in period_list:
			print("period",period)
			print("account names",row["account_name"])
			#print("d.get(period.key)",d.get(period.key))
			#if row["account_name"]=="Closing Stocks" and period['from_date']==datetime.date(2022, 7, 1):
				#print("entered")
				#period['from_date']=datetime.date(2022, 9, 1)
				#period['to_date']=datetime.date(2022, 9, 30)
				#print("---period",period)
			if d.get(period.key) and balance_must_be == "Credit":
				# change sign based on Debit or Credit, since calculation is done using (debit - credit)
				d[period.key] *= -1
				print("d[period.key]",d[period.key])
